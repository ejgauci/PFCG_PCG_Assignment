   ...
   // public List<Material> materialsList;
    ...
    void Start()
    {
        ...
        for (int i = 1; i < points.Count + 1; i++)
        {
            ...
            ExtrudeRoad(mg, pPrev, pCurr, pNext);
        }
    ...
    private void ExtrudeRoad(MeshGenerator mg, Vector3 pPrev, Vector3 pCurr, Vector3 pNext)
    {
        //Road Line
        Vector3 offset = Vector3.zero;
        Vector3 targetOffset = Vector3.forward * lineWidth;

        MakeRoadQuad(mg, pPrev, pCurr, pNext, offset, targetOffset, 0);

        //Road
        offset += targetOffset;
        targetOffset = Vector3.forward * roadWidth;

        MakeRoadQuad(mg, pPrev, pCurr, pNext, offset, targetOffset, 1);
        ...
        MakeRoadQuad(mg, pPrev, pCurr, pNext, offset, targetOffset, stripeSubmesh);
    ...
    //4. Create each quad
    private void MakeRoadQuad(MeshGenerator mg, Vector3 pPrev, Vector3 pCurr, Vector3 pNext,
                              Vector3 offset, Vector3 targetOffset, int submesh)
    {
        ...
        mg.BuildTriangle(topLeft, topRight, bottomLeft, submesh);
        mg.BuildTriangle(topRight,bottomRight,bottomLeft, submesh);
        ...
        mg.BuildTriangle(bottomLeft, bottomRight, topLeft, submesh);
        mg.BuildTriangle(bottomRight, topRight, topLeft, submesh);
    }
}
